...
AWS Credentials (injected at runtime)
      │
      ▼
aws eks update-kubeconfig  ──►  generates /tmp/kubeconfig (temporary)
      │
      ▼
kubectl apply -f mahesh.yml  ──►  EKS Cluster
      │
      ▼
rm -f /tmp/kubeconfig  ──►  cleanup, nothing stored permanently
                git branch: 'main', url: 'https://github.com/shivaprabha2997/Electro_Bootstrap.git'
            }
        }
        stage('Build Image') {
